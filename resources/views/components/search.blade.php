...
<body class="bg-gray-100">
    ...
    <div class="font-sans p-6 mx-auto xl:max-w-[1700px] lg:max-w-[1500px] md:max-w-[1100px] sm:max-w-[800px] max-w-full">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            @foreach (searchStamps as $searchStamp)
                    ...
                    <div class="bg-white border border-[#d1c7bd] rounded-lg overflow-hidden shadow-md hover:shadow-xl transform transition duration-300 hover:scale-105">
                        ...
                        <div class="p-6">
                            ...
                            <p class="text-sm font-bold text-[#966343] uppercase mb-2">Publicada el
                                {{ $searchStamp->created_at ? $searchStamp->created_at->format('d M Y') : 'Fecha no disponible' }}
                            </p>

                            <!-- Título -->
                            <a href="{{ route('detail', $searchStamp->id) }}" class="text-xl font-bold text-gray-700 mb-4 hover:text-[#7b4e33]">
                                {{ $searchStamp->name }}
                            </a>
                            <!-- Descripción -->
                            <p class="text-[#4a4a4a] mb-6">{{ $searchStamp->description }}</p>

                            <!-- Precio -->
                            <p class="text-2xl font-bold text-[#3D5300] mb-4">${{ number_format($searchStamp->price, 0, ',', '.') }}</p>

                            <!-- Botón -->
                            <a href="{{ route('detail', ['id' => $searchStamp->id]) }}">
